...
module.exports.run = async (bot, message, args) => {
    ...
    if(!helpArgs[0]) {
        var embed = new Discord.MessageEmbed()
            ...
            .setDescription('```player | bedwars  | duels | invite | help```')
            .addFields({ name: 'Prefix', value: settings.prefix, inline: true})
            .setColor('#00FFF3')
    ...
    if(helpArgs[0]) {
        ...
        if(bot.commands.has(command)) {
            ...
            - **Command's Permissions:** __${command.config.accessableby || "Everyone"}__
